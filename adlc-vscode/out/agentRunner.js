...
class AgentRunner {
    constructor(kitPath, projectMgr, memoryMgr) {
    ...
    setTerraformManager(terraform) {
        this.terraform = terraform;
    }
    setRasoolManager(rasool) {
        this.rasool = rasool;
    }
    setKiranManager(kiran) {
        this.kiran = kiran;
    }
    async launchAgent(agentName) {
        ...
        if (!model) {
            vscode.window.showErrorMessage(`GitHub Copilot model "${modelId}" not available. Make sure GitHub Copilot is installed and signed in.`);
            return;
        }
    ...
    buildContextBlock(agentName, pr, req, memory) {
    ...
    async applyAgentOutput(agentName, pr, response, out) {
    ...
    applyJSONPatches(agentName, pr, response) {
        // Group chat messages
        ...
        if (sm) {
            this.setAgentStatus(agentName, sm[1], parseInt(sm[2]), 'Updated by agent');
        }
    }
    // ── Helpers ───────────────────────────────────────────────────────────────
    setAgentStatus(agentName, status, progress, task, blocker = '') {
        const pr = this.projectMgr.getProjectRoot();
