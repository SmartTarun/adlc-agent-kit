...
class KiranManager {
    ...
    // ── API needs inference ───────────────────────────────────────────────────
    inferApiNeeds(req, modelFiles, componentSpec) {
        ...
        if (routers.length === 1) {
            if (/anomal|cost|alert/.test(combined))
                routers.push('anomalies', 'alerts');
            if (/user|auth|login|signup/.test(combined))
                routers.push('users', 'auth');
            if (/report|dashboard|metric/.test(combined))
                routers.push('reports', 'metrics');
            if (/property|lease|real.?estate/.test(combined))
                routers.push('properties', 'leases');
            if (/product|catalog|inventory/.test(combined))
                routers.push('products', 'inventory');
            if (/order|transaction/.test(combined))
                routers.push('orders');
            if (/notification|alert/.test(combined))
                routers.push('notifications');
            if (/budget|spend|billing/.test(combined))
                routers.push('budgets');
            if (/resource|infra|cloud/.test(combined))
                routers.push('cloud-resources');
        }
    ...
    // ── Prompt builder ────────────────────────────────────────────────────────
    buildPrompt(req, modelFiles, componentSpec, sfSchema, needs) {
...
exports.KiranManager = KiranManager;
//# sourceMappingURL=kiranManager.js.map