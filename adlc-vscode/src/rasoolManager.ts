...
export interface SnowflakeConfig {
  ...
  passphrase: string;   // may be empty
}

interface SchemaNeeds {
  dialect:      'postgresql' | 'snowflake' | 'both';
  entities:     string[];   // table/object names inferred from requirement
  needsAudit:   boolean;    // created_at / updated_at / deleted_at columns
  needsMultiTenant: boolean;
  needsTimeSeries: boolean; // time-series partitioning for analytics
  region:       string;
}

export class RasoolManager {
  private sfConfig: SnowflakeConfig | null = null;
  ...
  private async generatePostgres(
    model: vscode.LanguageModelChat,
    out: vscode.OutputChannel,
    token: vscode.CancellationToken,
    pr: string, req: any, db: any, needs: SchemaNeeds,
  ): Promise<void> {
  ...
  private inferSchemaNeeds(req: any, db: any): SchemaNeeds {
    ...
    if (/anomal|cost|spend|budget|billing|invoice/.test(combined))  entities.push('cost_anomalies', 'cost_alerts', 'budgets');
    if (/tenant|multi.?tenant|organization|org/.test(combined))     entities.push('organizations', 'tenants');
    if (/project/.test(combined))                                   entities.push('projects');
    if (/report|analytics|dashboard|metric/.test(combined))         entities.push('reports', 'metrics');
    if (/asset|resource|infra/.test(combined))                      entities.push('cloud_resources');
    if (/lease|property|real.?estate|building/.test(combined))      entities.push('properties', 'leases', 'tenants_re');
    if (/product|catalog|sku|inventory/.test(combined))             entities.push('products', 'inventory');
    if (/order|cart|purchase|transaction/.test(combined))           entities.push('orders', 'order_items', 'transactions');
    if (/audit|log|event|activity/.test(combined))                  entities.push('audit_logs');
    if (/notification|alert|email/.test(combined))                  entities.push('notifications');
    if (/tag|label|category|classif/.test(combined))                entities.push('tags', 'categories');

    const primary   = (db.primary   || 'postgresql').toLowerCase();
    const analytics = (db.analytics || 'none').toLowerCase();

    let dialect: 'postgresql' | 'snowflake' | 'both';
  ...
  private buildPostgresPrompt(req: any, db: any, needs: SchemaNeeds): string {
...
Generate all entity models and schemas. Use proper foreign keys and indexes.`;
  ...
  private buildSnowflakePrompt(req: any, db: any, needs: SchemaNeeds, sfHint: string): string {
...
Generate all tables with proper Snowflake DDL, keypair connection code, and documentation.`;
  ...
  static async loginWizard(context: vscode.ExtensionContext): Promise<SnowflakeConfig | undefined> {
    ...
    vscode.window.showInformationMessage(
      `ADLC: Snowflake keypair credentials saved for ${user}@${account}`,
    );
  ...
  async loadSnowflakeConfigFromSecrets(context: vscode.ExtensionContext): Promise<void> {
  ...
  getSnowflakeConnectionInfo(): string | null {
    if (!this.sfConfig) { return null; }
