...
export class FigmaDesigner {
  ...
  constructor(
    ...
    this.figmaToken = vscode.workspace.getConfiguration('adlc').get<string>('figmaToken') || '';
  }

  // ── Main entry — called from chatParticipant /ux or agentRunner kavya ────

  async runUXFlow(stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
    ...
    this.postToGroupChat(pr, 'KAVYA', 'UX Designer', 'discovery',
      this.formatQuestionsMessage(req),
      ['discovery', 'tarun', 'kavya']
    );
  ...
  async applyAnswersAndDesign(
    ...
    if (!data.teamDiscovery) { data.teamDiscovery = {}; }
  ...
  private async createFigmaDesigns(
    req:    any,
    stream: vscode.ChatResponseStream,
    token:  vscode.CancellationToken,
  ) {
    ...
    try {
      ...
      // Post to group chat
      this.postToGroupChat(pr, 'KAVYA', 'UX Designer', 'ux-design',
        `🎨 Figma designs ready! View here: ${figmaUrl}\n\nPlease review and share your feedback.`,
        ['tarun', 'kavya']
      );
  ...
  private async createFigmaFile(projectTitle: string, spec: any): Promise<string> {
    // Figma REST API — create a new file via drafts
    const { default: https } = await import('https');
    const body = JSON.stringify({
      name: `ADLC — ${projectTitle} — UX Design`,
      document: {
        children: spec.screens.map((screen: any, i: number) => ({
          id:   `page:${i}`,
          name: screen.name,
          type: 'CANVAS',
          children: [],
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          try {
            ...
            if (json.key) { resolve(`https://www.figma.com/file/${json.key}`); }
            else { reject(new Error(json.message || 'Unknown Figma API error')); }
          } catch { reject(new Error('Failed to parse Figma response')); }
        });
      });
      req.on('error', reject);
      req.write(body);
  ...
  private askForFeedback(stream: vscode.ChatResponseStream, figmaUrl: string | null, spec: any) {
    stream.markdown(`---\n\n### 📋 Design Overview\n\n`);
    ...
    spec.screens.forEach((s: any) => stream.markdown(`- **${s.name}** — ${s.description}\n`));
  ...
  async applyFeedback(feedback: string, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
    const pr  = this.projectMgr.getProjectRoot();
    const req = this.projectMgr.getRequirement();
  ...
  private async writeDesignTokens(pr: string, req: any, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
    ...
    const messages = [
      vscode.LanguageModelChatMessage.User(
        `You are Kavya, UX Designer. Based on this design spec, write a complete tokens.css file.\n` +
    ...
    this.postToGroupChat(pr, 'KAVYA', 'UX Designer', 'message',
      `✅ UX design tokens written to frontend/src/tokens/tokens.css. Component spec in docs/component-spec.md. Rohan can now build components.`,
      ['rohan', 'arjun']
    );
  ...
  private buildDesignSpec(req: any): any {
  ...
  private inferScreens(req: any, a: Record<string, string>): any[] {
  ...
  private defaultScreens(req: any): any[] {
  ...
  private buildComponentSpec(spec: any): string {
    return [
  ...
  private formatQuestionsMessage(req: any): string {
    return [
  ...
  private parseAnswers(raw: string) {
    const lines = raw.split('\n').map(l => l.replace(/^\d+[\.\)]\s*/, '').trim()).filter(Boolean);
    UX_QUESTIONS.forEach((q, i) => {
      if (lines[i]) { this.answers[q.key] = lines[i]; }
    });
  }

  private writeDesignSpec(pr: string, spec: any) {
    const file = path.join(pr, 'docs', 'ux-spec.json');
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, JSON.stringify(spec, null, 2), 'utf8');
  }

  private postToGroupChat(pr: string, from: string, role: string, type: string, message: string, tags: string[]) {
